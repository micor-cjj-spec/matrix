...
public interface BusinessEventOutboxMapper extends BaseMapper<BusinessEventOutboxEntity> {
    ...
    @Select("""
            SELECT *
              FROM matrix_erp_business_event_outbox
             ...
             LIMIT #{limit}
            """)
    List<BusinessEventOutboxEntity> findDue(@Param("now") LocalDateTime now, @Param("limit") int limit);

    @Update("""
            UPDATE matrix_erp_business_event_outbox
             ...
             WHERE fid = #{fid}
               ...
               AND (fnext_retry_time IS NULL OR fnext_retry_time <= #{now})
            """)
    int claim(@Param("fid") Long fid, @Param("claimToken") String claimToken, @Param("now") LocalDateTime now);
    ...
    @Update("""
            ...
            """)
    int markPublished(@Param("fid") Long fid, @Param("claimToken") String claimToken, @Param("now") LocalDateTime now);
    ...
    @Update("""
            ...
            """)
    int recoverStaleClaims(@Param("deadline") LocalDateTime deadline, @Param("now") LocalDateTime now);
}
