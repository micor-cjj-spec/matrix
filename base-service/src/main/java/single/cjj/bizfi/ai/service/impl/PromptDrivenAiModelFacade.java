...
public class PromptDrivenAiModelFacade implements AiModelFacade {
    ...
    public PromptDrivenAiModelFacade(AiPromptDocService promptDocService) {
    ...
    private AiModelResult callGeminiNativeModel(AiModelRequest request) throws Exception {
        ...
        if (StringUtils.hasText(systemPrompt)) {
            ...
            promptPart.put("text", "系统约束：\n" + systemPrompt);
            Map<String, Object> promptContent = new HashMap<>();
            promptContent.put("role", "user");
            promptContent.put("parts", Collections.singletonList(promptPart));
