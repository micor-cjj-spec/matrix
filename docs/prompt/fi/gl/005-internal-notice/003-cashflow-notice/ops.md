...
## 目标
根据 `docs/bus/fi/gl/005-internal-notice/003-cashflow-notice/` 下业务文档，以及当前已确认实现，输出现金流通知单场景的发布、运维、监控与问题处理提示词。
...
## 输出要求
1. 输出发布依赖检查项（前端/后端/现金流补充资料依赖）
2. 输出通知查询、生成、warnings、成功提示区相关监控关注点
3. 输出跳转到凭证页/现金流通知单勾稽页面兼容性注意项
4. 输出回滚与异常排查建议
