...
## 目标
根据 `docs/bus/fi/gl/005-internal-notice/003-cashflow-notice/` 下业务文档，以及当前前后端已确认实现，输出现金流通知单场景的测试与验证提示词。
...
## 输出要求
1. 覆盖初始化自动查询与查询场景
2. 覆盖通知状态、紧急程度、问题来源、期间过滤场景
3. 覆盖生成通知单、成功提示区、warnings、表格展示场景
4. 覆盖跳转到凭证页/现金流通知单勾稽页面、空结果、异常结果场景
