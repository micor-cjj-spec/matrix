...
## 目标
根据 `docs/bus/fi/gl/005-internal-notice/002-counterparty-notice-check/` 下业务文档、当前 prompt 文档，以及已确认实现，输出往来通知单勾稽场景的一致性评审提示词。
...
2. 检查仍需处理、已自然解除、快照未清、当前未清、warnings 口径是否一致
3. 检查查询、表格展示、跳转到往来对账单并决定 `openOnly=true` 是否一致
