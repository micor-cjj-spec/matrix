...
## 目标
根据 `docs/bus/fi/gl/004-counterparty-management/005-counterparty-writeoff-log/` 下业务文档，以及当前已确认实现，输出往来核销日志场景的发布、运维、监控与问题处理提示词。
...
## 输出要求
1. 输出发布依赖检查项（前端/后端/日志与链接数据依赖）
2. 输出核销日志查询、warnings、批次/明细联动相关监控关注点
3. 输出按方案号下钻兼容性注意项
4. 输出回滚与异常排查建议
