...
## 目标
根据 `docs/bus/fi/gl/004-counterparty-management/003-counterparty-statement/` 下业务文档，输出往来对账单场景的接口契约提示词，为前后端联调、测试与评审提供统一接口口径。
...
3. 输出 `rows`、`recentLogs` 的结构与统计口径说明
4. 输出关联凭证跳转与核销日志跳转参数映射说明
