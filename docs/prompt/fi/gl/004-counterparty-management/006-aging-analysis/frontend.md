...
## 目标
结合 `docs/bus/fi/gl/004-counterparty-management/006-aging-analysis/` 下业务文档，以及当前前端已确认实现，输出账龄分析表场景的前端实现建议与补齐提示词。
...
同时结合当前已确认实现：
- 页面：`CounterpartyAgingAnalysisView.vue`
- 查询区：往来类型、统计日期
- 汇总卡片、告警区、表格区
- 页面只做分析展示，不直接执行核销
...
2. 说明查询行为、空结果与异常态展示
3. 说明 warnings、统计字段、账龄分桶、风险状态展示逻辑
4. 说明页面只读分析边界
